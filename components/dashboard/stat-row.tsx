...
export function StatRow({
  ...
  stats: Stat[];
  /**
   * How many cells the row holds. Four is the page-width stat row; three is
   * the half-width variant a Home widget uses (APP_UX_PROPOSAL.md §3.3 —
   * "Inbox pulse"), where three short numbers fit one line at every size and
   * the 2×2 fallback would leave a cell empty.
   */
  columns?: 3 | 4;
}) {
    ...
    >
      {stats.map((stat, i) => (
        <div
          key={stat.key}
          className={cn(
            // Tighter cells on a phone, where the numbers share ~335px.
        ...
        >
          <div
            className={cn(
              ...
              // The three-cell row sits in a half-width widget: one size down,
              // so the figure doesn't crowd the eyebrow under it.
              three ? "text-2xl md:text-3xl" : "text-3xl lg:text-4xl"
            )}
          >
            {stat.value}
          </div>
